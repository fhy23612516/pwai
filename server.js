...
function buildSystemPrompt(kind) {
  const sceneGuidance = {
    ...
    review: [
      ...
      "如果 important_notes、boss_profile 或本单记录出现谈恋爱、暧昧、喜欢你、私下联系方式、线下见面、奔现等内容，profileUpdate 必须把它写成关系边界风险：memory_direction 写温和设边界后转回游戏，memory_risks 写不要恋爱承诺、暧昧升级、私联或线下承诺，memory_next_probe 写下次观察是否继续推进关系。",
  ...
  return [
    ...
    "如果 boss_profile、老板记忆或 payload 出现谈恋爱、暧昧、喜欢你、私下联系方式、线下见面、奔现等内容，必须识别为关系边界风险，而不是普通偏好；输出要分析风险、给温和边界话术，并把话题转回游戏或服务体验。",
    "关系边界风险场景禁止输出恋爱承诺、暧昧升级、私人联系方式交换、线下见面承诺，也不能用感情回应刺激消费。",
    `当前场景：${kind}`,
