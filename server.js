...
function buildSystemPrompt(kind) {
  const sceneGuidance = {
    ...
    simulate: [
      ...
      "emotionShift 写成陪玩能扫一眼懂的小抄，1-2 个短句，不要出现“情绪变化、用户需求、判断为、说明了”等报告词。",
      "readSignal 写成口语观察，比如“他是在试你会不会接亲近感”“他现在不想解释太多”，不要写成心理分析报告。",
      "nextSuggestion 直接给下一句能说的话，用“可以说：...”开头即可，不要写“建议你可以、策略、方案”。",
      "avoid 给 2-4 条短提醒，不要每条都写括号原因，不要写成长篇风控说明。",
      "simulate 的分析字段也要去 AI 味：不要用“信号解读、关系推进、情绪价值、建立连接、当前状态、复购信号、长期记忆命中、对话连续性”等术语。",
    ],
    review: [
  ...
  return [
    ...
    "减少 AI 味：不要使用“首先、其次、综上、赋能、情绪价值、建立连接、破冰、建议你可以、高质量陪伴、信号解读、当前状态、策略、方案、推进”等套话。",
